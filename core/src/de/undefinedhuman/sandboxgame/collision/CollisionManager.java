...
public class CollisionManager {
    ...
    public static boolean blockCanBePlaced(Entity entity, Vector2 blockPosition, byte cell) {
        ...
        if ((collisionComponent = (CollisionComponent) entity.getComponent(ComponentType.COLLISION)) != null) {
            ...
            Vector2 pos = new Vector2().add(entity.getPosition()).add(collisionComponent.getOffset());

            boolean collide = collide(pos, collisionComponent.getSize());

            if (collide) return false;
            World.instance.mainLayer.setBlock((int) blockPosition.x, (int) blockPosition.y, cell);
            collide = collide(pos, collisionComponent.getSize());
    ...
    // Position + offset
    public static boolean collide(Vector2 position, Vector2 size) {

        boolean collide = false;

        if (position.y < 0 || position.y + size.x > World.instance.height * Variables.BLOCK_SIZE) collide = true;

        for (int yPos = (int) (position.y / Variables.BLOCK_SIZE); yPos < Math.ceil(((int) position.y + size.y) / Variables.BLOCK_SIZE); yPos++)
            for (int xPos = (int) (position.x / Variables.BLOCK_SIZE); xPos < Math.ceil(((int) position.x + size.x) / Variables.BLOCK_SIZE); xPos++)
    ...
    public static boolean collideHike(Vector2 position, Vector2 size) {
        ...
        loop:
        for (int yPos = (int) (position.y / Variables.BLOCK_SIZE + 1); yPos < Math.ceil(((int) position.y + size.y) / Variables.BLOCK_SIZE); yPos++)
            for (int xPos = (int) (position.x / Variables.BLOCK_SIZE); xPos < Math.ceil(((int) position.x + size.x) / Variables.BLOCK_SIZE); xPos++)
