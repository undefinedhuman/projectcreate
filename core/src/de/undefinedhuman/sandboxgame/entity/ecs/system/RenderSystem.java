...
public class RenderSystem extends System {
    ...
    public void render(SpriteBatch batch) {
        ...
        for(Integer renderLevel : tempSpriteData.keySet())
            for (SpriteData data : tempSpriteData.get(renderLevel))
                if(data.isVisible()) { updateSpriteData(data); data.getSprite().draw(batch, data.getAlpha()); }
        ...
        if(Variables.renderHitboxes) for(Entity entity : EntityManager.instance.getEntities().values()) {
            ...
            if(collisionComponent != null) Tools.drawRect(batch,entity.getX() + collisionComponent.getOffset().x,entity.getY() + collisionComponent.getOffset().y, collisionComponent.getWidth(), collisionComponent.getHeight(), new Color(0.41568628f,0.3529412f,0.8039216f,0.4f));
        }
    ...
    private void setSpriteData(Entity entity, HashMap<Integer, ArrayList<SpriteData>> tempSpriteData) {
        ...
        if(transformComponent != null && spriteComponent != null) for(SpriteData data : spriteComponent.getSpriteData()) {
    ...
    private void updateSpriteData(SpriteData data) {
        ...
        sprite.setBounds(pos.x + data.getPositionOffset().x + data.getTurnedOffset().x, pos.y + data.getPositionOffset().y + data.getTurnedOffset().y, data.getSize().x != 0 ? data.getSize().x : data.entity.getWidth(), data.getSize().y != 0 ? data.getSize().y : data.entity.getHeight());
