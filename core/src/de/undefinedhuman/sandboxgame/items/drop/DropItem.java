package de.undefinedhuman.sandboxgame.items.drop;
...
public class DropItem {
    ...
    public void update(float delta) {
        ...
        if(this.target == null) {
            ...
            float dist = Math.abs(getCenter().dst(target.getCenteredPosition()));
            ...
            if(dist < 100) {
                ...
                if(dist < 25) {
                    ...
                    difference.set(new Vector2(getCenter()).sub(target.getCenteredPosition()));
    ...
    private Entity getNewTarget() {
        for(Entity player : EntityManager.instance.getPlayers()) if(Math.abs(player.getCenteredPosition().dst(getCenter())) < 80) return player;
        return null;
    }
