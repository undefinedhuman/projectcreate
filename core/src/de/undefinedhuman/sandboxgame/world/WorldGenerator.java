...
public class WorldGenerator {
    ...
    public World generateTestWorld(String name, WorldSetting worldSetting, BiomeSetting biomeSetting) {
    ...
    private void generateWorldBiomes(World world, BiomeSetting biomeSetting, Biome[] biomes, Noise[] noise, Noise caveNoise) {
        ...
        for(int y = 399; y >= 0; y--) { caveHeight[y] = 0.15f - (1f - caveNoise.linInterpolation(0,0.5f,(float) y / 399)) / 6; tempY[y] = maxHeight - y; }
        for(int x = 0; x < biomeTransitions.length; x++) biomeTransitions[x] = (float) x / biomeSetting.getTransition();
        for(int x = 0; x < borderSizes.length; x++) borderSizes[x] = calcBorders(x, borderSizes.length, borderSize);
        for(int i = 0; i < biomes.length; i++) generateBiome(world, tempY, biomeSetting, i, biomes, noise, caveNoise, list, borderSizes, caveHeight, biomeTransitions);
    ...
    private byte getBlockID(WorldLayer worldLayer, int x, int y, float[] borderSizes, Biome biome, ShiftList shiftList) {
        ...
        if(blockID != 0) for(Shift shift : shiftList.getShifts()) if(Tools.contains(blockID, shift.blockFilter) && shift.getNoise().select(shift.threshold,borderSizes[x] * shift.getNoise().fractal(x, y))) blockID = shift.blockID;
        return blockID;
    }
