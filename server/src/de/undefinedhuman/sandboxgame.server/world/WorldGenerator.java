        ...
        for (int i = 0; i < biomes.length; i++) {
            ...
            for (int y = 0; y <= height; y++) {
                ...
                for (int x = 0; x < biomeSetting.getSize(); x++) {
                    ...
                    if (value)
                        world.mainLayer.blocks[(biomeSetting.getSize() + biomeSetting.getTransition()) * i + x][maxHeight - y] = (byte) biomes[i].blockID;
                ...
                for (int x = 0; x < biomeSetting.getTransition(); x++) {
                    ...
                    if (!caveNoise.select(caveHeight, caveNoise.fractal((biomeSetting.getSize() + biomeSetting.getTransition()) * i + biomeSetting.getSize() + x, y)))
                        continue;
                        /*byte blockID = 0;
    ...
    public boolean checkWorldTile(World world, int x, int y) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            for (int j = -1; j < 2; j++) {
                ...
                if (j == 0) continue;
                if (getTileNeighbor(world, x, y, i == 0 ? j : 0, i == 1 ? j : 0)) solid++;
                else air++;
