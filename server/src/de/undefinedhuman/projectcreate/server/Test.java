...
import de.undefinedhuman.projectcreate.server.logger.BlockBreakEvent;
...
public class Test {
    ...
    public static void main(String[] args) throws IOException {
        ...
        String currentDate = Log.getTime();
        ...
        FileWriter writer = new FsFile("test-metadata", Files.FileType.Local).getFileWriter(false);
        writer
                .writeString(currentDate)
                .nextLine()
                .writeString(eventTypesUsed.toString())
                .nextLine()
                .writeString(uuidsUsed.toString())
                .close();
        writer = new FsFile("test", Files.FileType.Local).getFileWriter(false);
        for(String logLine : logLines)
            writer.writeString(logLine);
        writer.close();
        LZ4FrameOutputStream outStream = new LZ4FrameOutputStream(new FileOutputStream("test.lz4"));
        for (String element : logLines)
            outStream.write(element.getBytes(StandardCharsets.UTF_8));
        outStream.close();

        byte[] restored = new byte[decompressedLength];
        LZ4FrameInputStream inStream = new LZ4FrameInputStream(new FileInputStream("test.lz4"));
