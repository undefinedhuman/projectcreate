...
public class MovementSystem extends System {
    ...
    public void update(float delta, Entity entity) {
        ...
        if ((transformComponent = (TransformComponent) entity.getComponent(ComponentType.TRANSFORM)) != null && (movementComponent = (MovementComponent) entity.getComponent(ComponentType.MOVEMENT)) != null && (collisionComponent = (CollisionComponent) entity.getComponent(ComponentType.COLLISION)) != null && (animationComponent = (AnimationComponent) entity.getComponent(ComponentType.ANIMATION)) != null) {

            float currentX = transformComponent.getPosition().x, currentY = transformComponent.getPosition().y;
            ...
            if(left) { currentVel.x = -movementComponent.getSpeed(); } else if(right) { currentVel.x = movementComponent.getSpeed(); } else { currentVel.x = 0.0f; }
            ...
            } else { movementComponent.beginHike = false; }

            updateHitBoxPos(new Vector2(currentX, currentY), collisionComponent);
            if(CollisionManager.collide(hitboxPosition, collisionComponent.getWidth(), collisionComponent.getHeight())) { currentX = transformComponent.getPosition().x; }
            ...
            if (CollisionManager.collide(hitboxPosition, collisionComponent.getWidth(), collisionComponent.getHeight()) || hitboxPosition.y <= 0) {
                currentY = Math.max(transformComponent.getPosition().y, 0);
                currentVel.y = 0;
            }

            transformComponent.setPosition(currentX, currentY);
            ...
            else if(Tools.isInRange(currentVel.y, -jumpTrans, jumpTrans) && !movementComponent.canJump) animationComponent.setAnimation("TRANSITION");
            else if(currentVel.y < -jumpTrans && !movementComponent.canJump) animationComponent.setAnimation("FALL");
            else if (movementComponent.velocity.x != 0 || movementComponent.beginHike) animationComponent.setAnimation("RUN");
